...
This historical API is compiled only when `ASYNC_TCP_SSL_ENABLED` is provided
by a compatible transport. It is not available with the currently supported
ESP32Async dependency set.
...
This historical API has the same transport restriction as `setSecure()`.
